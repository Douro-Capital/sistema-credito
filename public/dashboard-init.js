...
// ── ESTADO GLOBAL (declarado aqui para evitar TDZ em qualquer onclick) ───────
...
let _EP = {exposicao_kw:[],comparar_kw:[],detalhe_kw:[],grafico_kw:[],evolucao_kw:[],sintese_kw:[],exposicao_ex:[],detalhe_ex:[],grafico_ex:[]};
...
// _EP inicializado em initDashboard() após dados carregarem
...
document.addEventListener('DOMContentLoaded', () => {
  function _waitForData(cb, attempts) {
    attempts = attempts || 0;
    if (window._dashboardLoaded) { cb(); return; }
    if (attempts > 200) { console.warn('[Dashboard] Timeout aguardando dados'); cb(); return; }
    setTimeout(function() { _waitForData(cb, attempts + 1); }, 50);
  }
  _waitForData(function() {
    _EP = _buildEmissorPhrases();
    if (_EP.exposicao_kw.length) _INTENT_KW.exposicao_setor    = (_INTENT_KW.exposicao_setor    || []).concat(_EP.exposicao_kw);
    if (_EP.comparar_kw.length)  _INTENT_KW.comparar_emissores = (_INTENT_KW.comparar_emissores || []).concat(_EP.comparar_kw);
    if (_EP.detalhe_kw.length)   _INTENT_KW.detalhe_ativo      = (_INTENT_KW.detalhe_ativo      || []).concat(_EP.detalhe_kw);
    if (_EP.grafico_kw.length)   _INTENT_KW.grafico_spread     = (_INTENT_KW.grafico_spread     || []).concat(_EP.grafico_kw);
    if (_EP.evolucao_kw.length)  _INTENT_KW.evolucao_fundamento= (_INTENT_KW.evolucao_fundamento|| []).concat(_EP.evolucao_kw);
    if (_EP.sintese_kw.length)   _INTENT_KW.sintese_emissor    = (_INTENT_KW.sintese_emissor    || []).concat(_EP.sintese_kw);
    if (_EP.exposicao_ex.length) _EXEMPLOS.exposicao_emissor   = (_EXEMPLOS.exposicao_emissor   || []).concat(_EP.exposicao_ex);
    if (_EP.detalhe_ex.length)   _EXEMPLOS.detalhe_ativo       = (_EXEMPLOS.detalhe_ativo       || []).concat(_EP.detalhe_ex);
    if (_EP.grafico_ex.length)   _EXEMPLOS.grafico_spread      = (_EXEMPLOS.grafico_spread      || []).concat(_EP.grafico_ex);
